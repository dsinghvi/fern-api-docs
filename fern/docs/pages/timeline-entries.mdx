...
The [timeline entries](https://devrev.docs.buildwithfern.com/dev-rev-rest-api/api-reference/timeline-entries/create) API is used to create discussions (comments) on the timeline of an object such as a ticket or issue. These messages can be sent by users or by a bot.
...
1. Get the [object ID](https://docs.devrev.ai/apis/getting-started#devrev-object-id) and object type where you want to send a message.

> Example: In issue `ISS-69`, `69` is the object ID and `ISS` indicates the object type `issue`.

2. Get the `dev-org-id` and `partition` by making a call to the [`dev-users.self` API](https://docs.devrev.ai/apis/getting-started#make-your-first-api-call).
...
![private-message](https://github.com/community/community/assets/74637789/52b40f71-6a6f-4022-8ea6-fc912f714480)