...
- [Getting Started](https://docs.devrev.ai/snap-ins/start): Deploying a snap-in
  to your dev org.
- [DevRev SDK](https://www.npmjs.com/package/@devrev/typescript-sdk): TypeScript
  SDK to ease in calling DevRev APIs.
...
The [Getting Started](https://docs.devrev.ai/snap-ins/start) section shows a
hello-world snap-in which prints a log message when a work item is created. This
snap-in has the following changes:
...
Do ensure that you have at least one part in your Organization since every
ticket must be linked to a part and have an owner.

### Trigger

The trigger condition for the snap-in is dictated by the
[Event Sources](https://docs.devrev.ai/snap-ins/references/manifest#event-sources)
section in the manifest. The [`timer-events`](https://docs.devrev.ai/snap-ins/references/event_sources#timerbased-event-sources)
...
Initialize the snap-in template for the hello-world snap-in.
...
As discussed in the [Getting Started](https://docs.devrev.ai/snap-ins/start),
the `code` folder contains the starter code for the snap-in.
...
[documentation](https://docs.devrev.ai/snap-ins/references/event_sources#timerbased-event-sources).
...
#### Using the DevRev SDK

Since a DevRev ticket is created whenever the function is triggered,
[DevRev APIs](https://docs.devrev.ai/api/) need to be called that allow this
...
[Getting Started](https://docs.devrev.ai/snap-ins/start) section.
