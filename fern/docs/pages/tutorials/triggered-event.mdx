...
- [Snap-in that takes an action](./timer-ticket-creator.mdx)

## Objective

This section enables users to create a dynamic snap-in that responds to both [DevRev Webhook] events triggered by the creation of a [work](../../product/core.mdx) and specialized [command](../references/commands.mdx) within DevRev. 
...
Following the manifest update, the next step is to incorporate the [event source](../references/event_sources.mdx).
...
To accommodate the additional trigger, a [command](../references/commands.mdx) must be defined within the
manifest file:
