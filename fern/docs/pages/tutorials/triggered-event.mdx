...
- [Snap-in that takes an action](/welcome/snap-in-development/tutorials/using-a-snap-in-to-perform-a-dev-rev-action)

## Objective

This section enables users to create a dynamic snap-in that responds to both DevRev Webhook events triggered by the creation of a [work](https://docs.devrev.ai/product/core) and specialized [command](/welcome/snap-in-development/references/commands) within DevRev. 
...
2. **Text command**: This manual trigger is achieved by utilizing a slash
   command in the **Discussion** section of the objects that support this feature.
...
As discussed in the previous sections, the initial step involves initializing
the snap-in template. This can be achieved using the following command:
...
Following the manifest update, the next step is to incorporate the [event source](/welcome/snap-in-development/references/event-sources).
...
To accommodate the additional trigger, a [command](/welcome/snap-in-development/references/commands) must be defined within the
manifest file:
