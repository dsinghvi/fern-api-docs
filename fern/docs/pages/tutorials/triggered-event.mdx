...
- [Snap-in that takes an action](/snap-in-development/tutorials/using-a-snap-in-to-perform-a-dev-rev-action)

## Objective

This section enables users to create a dynamic snap-in that responds to both DevRev Webhook events triggered by the creation of a [work](https://docs.devrev.ai/product/core) and specialized [command](/snap-in-development/references/commands) within DevRev. 
...
Following the manifest update, the next step is to incorporate the [event source](/snap-in-development/references/event-sources).
...
To accommodate the additional trigger, a [command](/snap-in-development/references/commands) must be defined within the
manifest file:
